...
if ( ! class_exists( 'TTW_Install_API' ) ) {

	/**
	 * Define TTW_Install_API class
	 */
	class TTW_Install_API {
		...
		public function do_theme_install() {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'Theme succesfully installed. Activating...', 'tm-theme-wizard' );
			...
			if ( is_wp_error( $result ) ) {
				...
				if ( ! isset( $skin->result->errors['folder_exists'] ) ) {
					...
					$message = esc_html__( 'Theme already installed. Activating...', 'tm-theme-wizard' );
				}
				...
				$message = esc_html__( 'Unable to connect to the filesystem. Please confirm your credentials.', 'tm-theme-wizard' );
		...
		public function do_plugin_install() {
			...
			$data    = array();
			$success = true;
			$message = esc_html__( 'Plugins wizard installed and activated. Redirecting...', 'tm-theme-wizard' );
		...
		public function adjust_theme_dir( $source, $remote_source, $upgrader ) {
			...
			if ( true === $wp_filesystem->move( $from_path, $to_path ) ) {
				 ...
				 */
				do_action( 'ttw_source_rename_done', $theme_data );
...
/**
 * Returns instance of TTW_Install_API
 *
 * @return object
 */
function ttw_install_api( $url = null ) {
	return TTW_Install_API::get_instance( $url );
}
