	...
	class TTW_Updater_API {
		...
		public function get_latest_release_link() {
			...
			if ( empty( $release_data ) ) {
				$this->error = esc_html__( 'Internal error, please try again later', 'tm-theme-wizard' );
				return false;
			}

			if ( isset( $release_data['error'] ) && true === $release_data['error'] ) {
				$this->error = esc_html__( 'Invalid Order ID', 'tm-theme-wizard' );
				return false;
			}
		...
		public function get_latest_release_version() {
			...
			if ( isset( $releases['errorMessage'] ) ) {
				$this->error = esc_html__( 'Template not found', 'tm-theme-wizard' );
				return false;
			}
